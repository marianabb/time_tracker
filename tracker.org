...
3. [X] Experiments with registration methods. After we know which method to use, since the registration approach depends on it.
...
- [X] For our Tensor-based method we must:
      - [X] Do intensity normalization. See http://www.itk.org/SimpleITKDoxygen/html/classitk_1_1simple_1_1N4BiasFieldCorrectionImageFilter.html
        - There doesn't seem to be a difference. Same exact results with and without normalization.
      - [ ] Try the registration method with smoothing after the normalization.

** DONE Meeting with Raili and Robin [2012-07-03 Tue]
   CLOSED: [2012-07-03 Tue 14:14]
   ...
   - We will focus on the voxel-based method for now because a Phd student will use it soon. The following suggestions
     were made:
     - [X] Raili suggests changing the color of the subtraction result to red (or something bright) and place it over
       the original image.
     - [X] Robin suggests having a LabelMap additional to the colored subtraction.
       - I decided to just leave the LabelMap since it's easier to handle and they both offer the same information.
     - [X] The result-volume doesn't seem to be the right way to go. Eliminate it.
     - [X] There are issues when the subtraction result is negative. Create my own subtraction module that returns 
     the *absolute value* of the subtraction.
   - [X] There will be another meeting with Raili on [2012-07-17 Tue]. Robin will probably not be present.
     - [X] I will prepare a laptop with Slicer + my module installed for Raili to test.
       - Not possible due to vacations. But I'll do the analysis myself and report to Raili.

** DONE Meeting with Raili [2012-07-17 Tue]
   CLOSED: [2012-07-17 Tue 17:15]
   - The laptop is not ready because I don't have the admin password.
   - [X] Raili gave me the MRIs of 18 patients to test the voxel based method.
** TODO Meeting with Robin [2012-07-31 Tue]
   - [X] Make a skeleton of what I'm going to write on the book. See [[Skeleton]].
   - [ ] Add amount of volume changed to Voxel-based method.
   - [X] Wrap up Tensor-based method. Jacobiant matrix or Jacobian determinant? Registration after smoothing?
** TODO Meeting with Robin [2012-08-27 Mon]
   - [ ] Plan and do the experiments.
     - Use one of the MRIs from Joel as "unchanging".
     - Modify slightly the same MRI from Joel and see how it behaves.
       - Modifications must be black parts or making some part bigger. 
       - Try with modifications of different sizes.
     - Tensor based method can have a few changeable variables:
       + [X] Displacement field smoothing sigma.
         - 2.5 is still a good number. Results with 1.0 or 1.5 usually give deformations fields not smooth enough.
       + [X] Percentage of the Jacobian to use (20%, 30%).
         - Different for high and low values: High = 22.5%, Low = 30%. Since there are much more high values.
   - [X] Apply log to the Jacobian before extracting the percentage to use.
   - [X] Try FEMRegistration but only if it works fast.
     - It didn't. Maybe will try it later.
** DONE Meeting with Raili and Robin [2012-08-28 Tue]
   CLOSED: [2012-09-17 Mon 23:08]
   - Suggestions for the experiments.
   - [X] Prepare a few GOOD images for the paper. Focus on the patients CL and PB.
     - Sent an email with a Dropbox link. 
     - PB's differences are too small.
** TODO Meeting with Robin [2012-09-14 Fri]
   - Suggestions to create the "artificial" tests.
   - [ ] Is there a way to count the size of the volume changes detected by the tensor method?
     - Maybe something with growth segmentation directly in Slicer. Initial tests not very successful.
...
* TODO Tensor-based Morphometry method
...
2. Jacobian Determinant [[http://en.wikipedia.org/wiki/Jacobian_determinant#Jacobian_determinant][here]]:
   +  if the Jacobian determinant at a point $p$ is positive, then $F$ preserves orientation near $p$; if it is negative, 
    $F$ reverses orientation. The *absolute value* of the Jacobian determinant at $p$ gives us the factor by which the function $F$ 
...
**** TODO Try the algorithm with other volumes. Begin with Meningioma1 and Meningioma2.
     ...
     3. Case batch2_3 ::
        - Image has some not-so-small differences.
        - BRAINSDemons (Diffeomorphic) and default settings: Good registration but the DF is not very smooth. The Jacobian values are 
          more varied: Max Jacobian = 18.6172, min Jacobian = -0.198405.
        - BRAINSDemons (Diffeomorphic) + Displacement field smoothing sigma 2.5: Smoother DF. Smaller Jacobian values:
          Max Jacobian = 3.54618, min Jacobian = 0.135685.
        - BRAINSDemons (Diffeomorphic) + Displacement field smoothing sigma 3: Smoother DF. Smaller Jacobian values:
          Max Jacobian = 2.59, min Jacobian = 0.27.
...
** DONE Add Affine registration to the options and to be the default method.
   CLOSED: [2012-07-16 Mon 00:12]
** DONE Find a measure of local intensity difference
   CLOSED: [2012-07-31 Tue 13:28]
So far only the absolute value of the subtraction.   
*** DONE There are issues when the subtraction result is negative. 
    CLOSED: [2012-07-16 Mon 15:01]
- Create my own subtraction module that returns the *absolute value* of the subtraction.
  - CLI Module created instead of ITKTest. It is now called ITKSubtractor.
** DONE Eliminate the result-volume
   CLOSED: [2012-07-16 Mon 00:12]
** DONE Change the color of the subtraction result to something bright
   CLOSED: [2012-07-16 Mon 00:12]
** DONE Subtraction with negative values AND create LabelMap!
   CLOSED: [2012-07-16 Mon 14:31]
Used a CLI module written by me with the itk filter /AbsoluteValueDifferenceImageFilter/.
** DONE Delete the threshold or do something with it
   CLOSED: [2012-07-16 Mon 14:57]
Just commented the part in case we need it later.
** DONE Maybe comment some of the registration methods available
   CLOSED: [2012-07-16 Mon 14:38]
- Commented out Fiducial registration.
- Ordered the methods in order of 'powerfullness'
** TODO Quitting crashes! Why?!.
- It is something in my module. Not surprisingly, it also happens with ChangeTracker. I guess I imported the error.
- It happens after I load volumes into the baseline and followup in the SelectVolumes step. Either by using the 'load tests' button or manually.

** DONE Prepare a laptop for Raili for the meeting of [2012-07-17 Tue]
   CLOSED: [2012-07-22 Sun 00:22]
- Might take more time that I expected since I have to compile Slicer version 4 (with ITKv4) on a linux machine and currently the only
  one available to me is /troubadix/ in the department which is VERY slow.
- [2012-07-17 Tue] I have the laptop 11. Windows 7, with no admin permissions which does not allow me to install anything. I wrote an email to Fredrik Nysjö, 
  Olle and Lennart.
- Cancelled due to vacation. We decided that I'll analyse 18 patients myself.
** DONE Analyse the 18 patients Raili gave me before August 8th.
   CLOSED: [2012-07-30 Mon 13:29]
- [2012-07-20 Fri] I made an awesome script to prepare the MRI images.
- CG :: Affine registration works.
        Can't see anything worth reporting.
- CL :: Affine registration doesn't produce a very good result. As a consequence, the differences seem to be more than what I believe
        they are. BSpline registration is no better.
        Demon registration produces a very good registration, but I can see a small diference in the top right part of the brain.
        Took screenshots.
- CS :: Affine registration changes the volume's dimensions in two of the orientations. Not a very useful result.
        BSpline registration produces a sligthtly better result (no dimension change), but still not a very good registration, which
        turns out on a bright (and probably not trustable) labelMap.
        Affine registration with different order seems to work. Can't see anything worth reporting.
- HK :: I think is neck guy! The MRIs are very well aligned.
        Affine registration shows a thin line of difference in the Corpus Callosum. Added a screenshot just in case.
- JN :: Pretty good initial positions.
        Affine registration: Not many differences, maybe a small line on the back of the brain. Added screenshot just in case.
- JO :: Affine registration looks fine.
        Can't see anything worth reporting.
- JS :: Affine registration works.
        Can't see anything worth reporting.
- KM :: The 2nd MRI is rotated, not a very usual problem so far. Affine transformation doesn't completely fix this.
        Produces a 'ghost' of everything in the LabelMap, so it's quite hard to see real differences.
        BSpline registration produces a better result, but still is hard to differenciate the real differences from the
        registration issues.
        [2012-08-01 Wed] Second try: Affine registration with 50 histogram bins, 3000 iterations and 200000 spatial samples. Better result
        took some screenshots of possible differences.
- KS :: Affine registration works.
        Can't see anything worth reporting.
- ME :: Affine regiatration looks fine.
        Can't see anything worth reporting.
- MLe :: Affine registration works. There are numbers on the forehead of the patient.
         Can't see anything worth reporting.
- MLu :: Quite a different patient position between both exams.
         Affine registration has ghosts because of imperfect result. Can't know which ones are real differences.
         BSpline registration gives a terrible result.
         [2012-08-01 Wed] Second try: 100 Histogram bins, 400000 spatial samples and 5000 iterations. Saw some possible differences
         and took screenshots.
- MT :: Affine registration works.
        Sort of brigther that usual for a good affine registration, but can't see anything worth reporting.
- NN :: Affine registration works.
        Can't see anything worth reporting.
- PB :: Affine registration looks fine. Very small possible difference. Took screenshot.
        Demon registration doesn't show the small difference.
- SB :: Terrible first MRI. The patients nose is outside the image.
        Affine registration is not enough.
        BSpline registration is not enough.
        Demon registration produces a very good result as usual, but not many differences. I took a screenshot.
        [2012-08-01 Wed] Second try: Affine registration 50 histogram bins, 800000 spatial samples, 3000 iterations.
        The registration is not perfect but I can still see something that resembles a brighter difference. I took screenshots.
- SI :: Second MRI with part of the patient outside the image.
        Affine registration 'squishes' the resulting image in 2 dimensions. Affine registration with MRI2 as fixed volume works much
        better. The final LabelMap, although pinkinsh, shows no small differences.
        BSpline registration is not enough. In both orders.
        Demon registration produces some bright lines. I don't think they are real differences.
- SS :: The angle is very different between both MRIs.
        Affine registration is not enough.
        BSpline registration is not enough.
        Demons registration produces no differences in the LabelMap.
        [2012-08-01 Wed] Second try: Affine registration with 200000 spatial samples. Worked, no differences worth reporting.
- [2012-07-30 Mon] Sent my results to Raili.
...
** TODO Paper "Voxel-based morphometry - The methods"
- They include an extra smoothing step before calculating differences. Maybe I don't need it because I'm analysing the same
  person. But it could help.
  
** DONE Write a short description of the method for Marianne's paper
   CLOSED: [2012-08-07 Tue 18:38]
The computer-aided volume comparison method was developed from already existent ideas on Voxel-based morphometry. It works by
registering the first MRI of the patient to the second using transformations that preserve straight lines, and ratios of distances 
between points lying on a straight line, namely affine transformations. Once the two volumes are properly aligned, the program
subtracts the first MRI (assumed to be the fixed-volume, or base) from the volume created by the registration. The result is
another volume that indicates which voxels have changed the most according to the subtraction values.
Since the software results might have been biased by the registration process, specially in cases where the patient's position changed
significantly from one MRI to the other, it was not the only method used to determine if a volume was indeed different from 
the first MRI to the second. Instead, it was used as guidance for one experienced neuroradiologist in a second round of analysis,
after having reviewed the patients without computerized help.

- Description after Robin's input:
  The computer-aided volume comparison method was developed from already existent ideas on Voxel-based morphometry. It works by 
  registering the first MRI of the patient to the second using affine transformations, i.e., transformations that preserve straight lines 
  and ratios of distances between points lying on a straight line. Once the two volumes are properly aligned, the program subtracts  
  the first MRI (assumed to be the fixed-volume, or base) from the volume created by the registration. The result is another volume 
  that indicates which voxels have changed the most according to the subtraction values.

  In a first round analysis, an experienced neuroradiologist analyzed the patients image data and determined if a volume was  
  different from the first MRI to the second without the computer-aided volume comparison method. The software results were used as 
  guidance for the neuroradiologist in a second round of analysis in which he or she also used the original image data. 
  Note that the software results might in some cases have been biased by the registration process, specially in cases where the 
  patient's position changed significantly from one MRI to the other.
...
** Skeleton
   1. Introduction
      ...
      - Other works (in passing)
      - Comparison of other methods
   2. Methods
      1. Registration
         * Affine registration
         * BSpline registration
         * BRAINS Demon warp registration 
      2. Morphometry
         * Voxel-based morphometry
         * Tensor-based morphometry
         * Deformation-based morphometry (Maybe)
   3. Proposed solutions
      ...
      3. Tensor-based method
   4. Experiments
      * Type of data used
      * Types of differences added
      * Example
      1. Voxel-based results
      2. Tensor-based results
   5. Applications
      * Raili's study
      * Other projects
   6. Conclusions and future works
  
** [2012-09-09 Sun] Thesis repository created
- Here: https://github.com/marianabb/thesis_book


** Experiments
- Use one (or two) of Joel's MRIs with no changes (find one with no easy-detectable differences).
   - Add differences of changing sizes. I am still not sure how to add the differences, last time I used Gimp and did it manually, perhaps Matlab might be a better idea.
   - At least 3 different sizes of differences.  The smallest ones should be pretty small, just to try.
- Take two or three MRIs with known differences and compare the voxel-based with the tensor-based method. See if they find different results.
   - The ones suggested by Raili for the paper (CL and PB).
   - The one we usually use because it has big differences (batch2_3).
...
** TODO SPM Statistical Parametric Mapping
- Web: http://www.fil.ion.ucl.ac.uk/spm/
- Written by UCL. Voxel-based morphometry. 
- The one that comes out in Wikipedia.
- Made in MATLAB
