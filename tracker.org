...
Current goals:
- [ ] Finish adding registration methods.
- [ ] Add a field for an already registered volume.
- [ ] Create a third step for quantification that only substract the volumes.
- [ ] Show the differences as a different color/intensity on the screen.
...
*** TODO Add Fiducial registration
